...
export const SettingsLogin = () => {
	...
	const activateTwoFactor = async () => {
		...
		try {
			await axios.post("/wocman/settings/activate/2fa");
		} catch (error) {}
	};
	...
	const cancelTwoFactor = async () => {
		setSettings({ ...settings, twofactor: !settings.twofactor });
		try {
			await axios.post("/wocman/settings/cancel/2fa");
		} catch (error) {}
	};
	...
	const activateDeviceSettings = async () => {
		...
		try {
			await axios.post("/wocman/settings/activate/ipa");
		} catch (error) {}
	};
	...
	const cancelDeviceSettings = async () => {
		setSettings({ ...settings, deviceSettings: !settings.deviceSettings });
		try {
			await axios.post("/wocman/settings/cancel/ipa");
		} catch (error) {}
	};
	...
	const changePassword = async (body, resetForm) => {
		...
		try {
			const { data } = await axios.post("/wocman/settings/edit/password", body);
			if (data?.status) {
				resetForm();
				ShowMessage(
					"Success",
