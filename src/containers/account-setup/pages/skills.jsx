...
const SkillLevel = ({ step, setStep, prevStep }) => {
  ...
  const handleCertficateChange = async (e) => {
    ...
    if (fileTypes.includes(file?.type)) {
      ...
      ShowMessage("Error", "An error occurred. try again", "error", toast);
      setCertificate("");
    }
    console.log(form);
  };
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      const { data } = await axios.post("/wocman/profile/add/competency", {
        competenceid,
      });
      if (data?.status === 200 || 201) {
        ShowMessage("Success", "profile completed, please login", "success", toast);
      }
      setIsLoading(false);
  ...
  const setCompetency = (item, index) => {
    setSkillLevel(item);
    setCompetenceid(index + 1);
  };
          ...
          >
            {[
              ...
              <LevelCard
                ...
                onClick={() => setCompetency(item.level, index)}
...
const LevelCard = (props) => (
  <Flex
    ...
    cursor="pointer"
    border={`${
      props.skillLevel === props.heading ? "5px solid #C96A54" : "none"
    }`}
  >
    <Flex
      flex="2"
    ...
    >
      ...
      >
        <Text
          ...
          py={2}
          color={`${
            props.skillLevel === props.heading ? "white" : "wocman.heading_text"
          }`}
        >
          {props.heading}
        </Text>
        <Image
          ...
          alt="check mark"
          mr={{ base: 2, md: 4 }}
          d={`${props.skillLevel === props.heading ? "flex" : "none"}`}
        />
      </Flex>
      <Flex
        flexDir="column"
      ...
      >
        <Flex>
          <Text
            ...
            lineHeight="138.6%"
            color={`${
              props.skillLevel === props.heading
                ? "white"
                : "wocman.heading_text"
            }`}
            // ml={{ base: 4, md: 8 }}
